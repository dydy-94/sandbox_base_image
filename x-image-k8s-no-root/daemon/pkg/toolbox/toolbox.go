...
func (s *Server) Start() error {
	...
	r.Use(middlewares.LoggingMiddleware(s.AuthConfig))
	r.Use(middlewares.ErrorMiddleware())

	// 本地 JWT 鉴权中间件（仅在配置启用时生效）
	if s.AuthConfig != nil && s.AuthConfig.Auth.Enabled {
		jwtAuth, err := middlewares.JWTAuthMiddleware(s.AuthConfig)
		if err != nil {
			log.Fatalf("Failed to init JWT auth middleware: %v", err)
		}
		r.Use(jwtAuth)
	} else {
		log.Info("JWT auth disabled")
	}
