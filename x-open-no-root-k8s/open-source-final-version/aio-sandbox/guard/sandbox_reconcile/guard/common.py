...
def log(level: str, event: str, message: str, **kwargs: Any) -> None:
    ...
    if os.environ.get(MIRROR_PID1_ENV, "").strip() == "1":
        ...
        try:
            # rootless (user=x) cannot write to root-owned /proc/1/fd/1.
            # Probe readability before opening so we don't spam permission
            # denied errors on every log line.
            if os.access(target, os.W_OK):
                with open(target, "a", encoding="utf-8") as fp:
                    fp.write(line + "\n")
                mirrored = True
            else:
                os.environ.pop(MIRROR_PID1_ENV, None)
        except Exception:
            pass
