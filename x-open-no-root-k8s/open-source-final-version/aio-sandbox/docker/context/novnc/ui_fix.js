...
const UI = {
    ...
    initSettings() {
        ...
        let frame_id =  window.location.pathname.replace(/^\//, '').split('/')[1];
        WebUtil.writeSetting('frame_id', frame_id);
        UI.initSetting('host', '');
 ...
 * ------v------*/
    ...
    clipboardReceive(e) {
        ...
        // 监控剪贴板：VNC 端剪贴板变化自动同步到浏览器系统剪贴板
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(e.detail.text).then(() => {
                Log.Debug("Successfully copied to system clipboard");
            }).catch(err => {
                Log.Error("Failed to copy to system clipboard: " + err);
            });
        } else {
            Log.Warn("Clipboard API not available (requires secure context)");
        }
    ...
    // 监控剪贴板：从浏览器系统剪贴板读取并发送到 VNC 端
    clipboardPaste() {
        if (navigator.clipboard && navigator.clipboard.readText) {
            navigator.clipboard.readText().then(text => {
                if (text !== document.getElementById('noVNC_clipboard_text').value) {
                    Log.Debug("Reading clipboard from system");
                    document.getElementById('noVNC_clipboard_text').value = text;
                    UI.clipboardSend();
                }
            }).catch(err => {
                Log.Error("Failed to read system clipboard: " + err);
            });
        } else {
            Log.Warn("Clipboard API not available (requires secure context)");
        }
 ...
 * ------v------*/
    ...
    connect(event, password) {
        ...
        const frame_id = WebUtil.readSetting('frame_id');
    ...
    disconnectFinished(e) {
        ...
        document.getElementById('noVNC_container').removeEventListener('click', UI.clipboardPaste);
        window.removeEventListener('focus', UI.clipboardPaste);
