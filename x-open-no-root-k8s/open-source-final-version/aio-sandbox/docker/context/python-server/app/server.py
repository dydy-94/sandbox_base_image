...
async def app_lifespan(app: 'FastAPI'):
    ...
    # v13-fix9.11: jupyter service removed.
    # base image bakes ENV DISABLE_JUPYTER=true (Dockerfile.offline L921);
    # when set, skip importing JupyterService so the startup NoSuchKernel
    # ERROR + 0-sessions warning (L152-174 in 80384013 log) never happens.
    # The try/except ImportError fallback is preserved for environments
    # where DISABLE_JUPYTER is unset / false and the jupyter optional deps
    # happen to be missing — those envs would previously crash hard at
    # startup with ImportError on `from app.services.jupyter`.
    JupyterService = None
    if os.environ.get('DISABLE_JUPYTER', '').lower() != 'true':
        try:
            from app.services.jupyter import JupyterService as _JupyterService
            JupyterService = _JupyterService
        except ImportError:
            JupyterService = None
    ...
    # v13-fix9.11: only register jupyter_service when DISABLE_JUPYTER is
    # not true (i.e. jupyter is not removed).  When DISABLE_JUPYTER=true,
    # JupyterService is None above and the /jupyter REST routes were also
    # skipped in app/api/router.py.
    if JupyterService is not None:
    ...
    try:
        ...
        # v13-fix9.11: only register the jupyter session pool when
        # jupyter_service is present (i.e. DISABLE_JUPYTER != true).
        # Previously this line crashed with KeyError when DISABLE_JUPYTER
        # was true and the service registration above was skipped.
