...
# v13-fix9.11: jupyter router removed.
# base image bakes ENV DISABLE_JUPYTER=true (Dockerfile.offline L921) and
# the supervisord python-server env also exports DISABLE_JUPYTER=true, so
# the v13-fix9.11 image variant of the open-source AIO sandbox ships
# WITHOUT jupyter support.  Skip importing app.api.v1.jupyter entirely so
# the L120 "jupyter router imported" log line never appears and the
# ipykernel / jupyter_client dependency on requirements/server.txt is no
# longer loaded at import time.  This eliminates the L152-174 startup
# NoSuchKernel ERROR + 0-sessions warning when no python3 kernel spec is
# baked.  Setting DISABLE_JUPYTER=false (or unsetting it) would
# re-enable the router, but requires also installing jupyter-client +
# registering an ipykernel kernel spec.
if os.environ.get('DISABLE_JUPYTER', '').lower() == 'true':
    logger.info('jupyter router skipped (DISABLE_JUPYTER=true)')
else:
    from app.api.v1 import jupyter  # noqa: F401
...
def register_routes(app: FastAPI):
    ...
    if os.environ.get('DISABLE_JUPYTER', '').lower() != 'true' and 'jupyter' in globals():
        # v13-fix9.11: only register the /jupyter router when jupyter is
        # NOT disabled AND the jupyter router module was actually imported
        # above (the `jupyter` name is only present in this module's
        # global namespace when DISABLE_JUPYTER != 'true').
