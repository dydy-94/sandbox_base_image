...
if [ ${#PORTS_ARRAY[@]} -eq 0 ] && [ ${#FILES_ARRAY[@]} -eq 0 ]; then
  ...
  while true; do
    ...
    current_time=$(date +%s)
    elapsed=$((current_time - start_time))
    # Default to 120s if WAIT_TIMEOUT is unset — bash's `[ -ge "" ]` errors out
    # with "unary operator expected" otherwise.
    _timeout="${WAIT_TIMEOUT:-120}"
    if [ "$elapsed" -ge "$_timeout" ]; then
      echo "$(date '+%Y-%m-%d %H:%M:%S') - Timeout after ${_timeout}s waiting for services"
